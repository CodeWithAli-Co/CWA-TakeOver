...
const CalculatorTab: React.FC<CalculatorTabProps> = ({
  ...
  const addRevenue = (): void => {
    const newId =
      revenues.length > 0 ? Math.max(...revenues.map((r) => r.id)) + 1 : 1;
    setRevenues([
      ...revenues,
      {
        id: newId,
  ...
  const updateExpense = (updatedItem: ExpenseItem): void => {
    setExpenses(
      expenses.map((item) => (item.id === updatedItem.id ? updatedItem : item))
    );
  };
  ...
  const updateRevenue = (updatedItem: RevenueItem): void => {
    setRevenues(
      revenues.map((item) => (item.id === updatedItem.id ? updatedItem : item))
    );
  };
  ...
  return (
    <div className="backdrop-blur-lg bg-black bg-opacity-40 border border-red-900 p-8 shadow-[0_0_30px_rgba(0,0,0,0.3)]">
      ...
      <div className="text-sm text-red-400 mb-8 font-mono">
        Configure business parameters for quantum financial analysis
      </div>

      {/* Sub Tabs for Calculator */}
      <Tabs value={calcSubTab} onValueChange={setCalcSubTab} className="mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm mt-2">
                      <div className="flex flex-col items-center p-2 border border-green-900 bg-green-900/10">
                        ...
                        <span className="font-mono">
                          ${projections[1].totalRevenue.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex flex-col items-center p-2 border border-red-900 bg-red-900/10">
                        ...
                        <span className="font-mono">
                          ${projections[1].totalExpenses.toLocaleString()}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-sm text-red-400">
                Configure all your business expenses and operational costs
              </p>
            </div>
              ...
              <div className="bg-black/30 border border-red-900 p-6">
                ...
                <div className="mb-6">
                  ...
                  <div className="text-sm text-red-400 mt-1">
                    + ${(avgSalary * employeeCount).toLocaleString()} in
                    employee costs
                  </div>
                </div>
                          ...
                          data={Object.entries(
                            expenses.reduce(
                              (acc, exp) => {
                                ...
                                acc[cat] =
                                  (acc[cat] || 0) + calculateAnnualAmount(exp);
                                return acc;
                              },
                ...
                <div className="mt-4 text-xs text-gray-500">
                  ...
                  <ul className="list-disc pl-4 space-y-1">
                    <li>
                      Group similar expenses into categories for better analysis
                    </li>
                    <li>
                      Set realistic growth rates based on industry averages
                    </li>
                    <li>
                      For recurring expenses, select the appropriate frequency
                    </li>
                  </ul>
                </div>
              </div>
        ...
        <TabsContent value="revenue" className="mt-0">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-sm text-red-400">
                Configure your business revenue sources and income streams
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="max-h-[600px] overflow-y-auto pr-2">
                {revenues.map((revenue) => (
                  <DynamicItem
                    key={revenue.id}
                    item={revenue}
                    // @ts-ignore (inserting revenue function here)
              ...
              <div className="bg-black/30 border border-red-900 p-6">
                ...
                <div className="mb-6">
                  ...
                  <div className="text-3xl font-bold text-white font-mono">
                    ...
                    {revenues
                      .reduce(
                        (total, revenue) =>
                          total + calculateAnnualAmount(revenue),
                        0
                      )
                      .toLocaleString()}
                    <span className="text-sm text-red-400 ml-2">/ year</span>
                          ...
                          data={Object.entries(
                            revenues.reduce(
                              (acc, rev) => {
                                ...
                                acc[cat] =
                                  (acc[cat] || 0) + calculateAnnualAmount(rev);
                                return acc;
                              },
                              {} as Record<string, number>
                            )
                ...
                <div className="mt-4 text-xs text-gray-500">
                  ...
                  <ul className="list-disc pl-4 space-y-1">
                    <li>
                      For recurring revenue, be realistic about growth rates
                    </li>
                    <li>
                      Consider different frequency options for each revenue
                      stream
                    </li>
                    <li>
                      For subscription models, estimate your client/user base
                    </li>
                  </ul>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              <div className="mt-6 bg-black/30 border border-red-900 p-4">
                <h4 className="text-red-400 mb-3">
                ...
                <div className="flex justify-between items-center text-sm mb-2 pt-2 border-t border-red-900/50">
                  ...
                  <span className="font-mono text-white">
                    ${(avgSalary * employeeCount).toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
                      ...
                      >
                        ...
                        <Tooltip
                          formatter={(value, name) => {
                            if (name === "employeeCost")
                              return [
                                `$${Number(value).toLocaleString()}`,
                                "Personnel Cost",
                              ];
                            if (name === "percentOfExpenses")
                              return [
                        ...
                        {projections.slice(1, 6).map((year) => (
                          ...
                          >
                            ...
                            <td className="py-2 text-right font-mono">
                              {year.totalExpenses > 0
                                ? (
                                    (year.employeeCost / year.totalExpenses) *
                                    100
                                  ).toFixed(1)
                                : 0}
                              %
