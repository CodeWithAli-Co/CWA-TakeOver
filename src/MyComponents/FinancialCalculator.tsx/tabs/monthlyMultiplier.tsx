  ...
  useEffect(() => {
    const loadValues = async () => {
      try {
        ...
        setItems([
          ...(expenseData?.map((e) => ({ ...e, itemType: "expense" })) || []),
          ...(revenueData?.map((r) => ({ ...r, itemType: "revenue" })) || []),
        ]);
      } catch (err) {
  ...
  const chartData = [
    {
      month: 1,
      ...items.reduce(
        (acc, item) => ({ ...acc, [item.name]: item.amount * 1 }),
        {}
      ),
    },
    {
      month: 3,
      ...items.reduce(
        (acc, item) => ({ ...acc, [item.name]: item.amount * 3 }),
        {}
      ),
    },
    {
      month: 6,
      ...items.reduce(
        (acc, item) => ({ ...acc, [item.name]: item.amount * 6 }),
        {}
      ),
    },
    {
      month: 12,
      ...items.reduce(
        (acc, item) => ({ ...acc, [item.name]: item.amount * 12 }),
        {}
      ),
    },
    {
      month: 24,
      ...items.reduce(
        (acc, item) => ({ ...acc, [item.name]: item.amount * 24 }),
        {}
      ),
    },
    {
      month: 36,
      ...items.reduce(
        (acc, item) => ({ ...acc, [item.name]: item.amount * 36 }),
        {}
      ),
    },
    {
      month: 60,
      ...items.reduce(
        (acc, item) => ({ ...acc, [item.name]: item.amount * 60 }),
        {}
      ),
    },
  ];
        ...
        <div>
          <div className="mb-6">
            ...
            {items.map((item, index) => (
              <div
                key={index}
                className={`bg-${item.type === "expense" ? "red" : "green"}-950/10 border border-${item.type === "expense" ? "red" : "green"}-900 p-3 mb-3 relative`}
              >
                <button
                ...
                <div className="mb-2">
                  <label
                    className={`text-xs text-${item.type === "expense" ? "red" : "green"}-400`}
                  >
                    Name
                  </label>
                  <input
                ...
                <div>
                  ...
                  <div className="flex">
                    <span
                      className={`bg-${item.type === "expense" ? "red" : "green"}-900/20 border-y border-l border-${item.type === "expense" ? "red" : "green"}-900 px-2 flex items-center text-${item.type === "expense" ? "red" : "green"}-400`}
                    >
                      $
                    </span>
                    <input
          ...
          <div className="grid grid-cols-3 gap-3 mt-4">
            ...
            <div
              className={`${calculateNet() >= 0 ? "bg-red-950/10 border border-red-900" : "bg-red-950/10 border border-red-900"} p-3`}
            >
              <div
                className={`text-xs ${calculateNet() >= 0 ? "text-red-400" : "text-red-400"} mb-1`}
              >
                Net Total
              </div>
              <div
                className={`text-xl font-bold ${calculateNet() >= 0 ? "text-red-400" : "text-red-400"} font-mono`}
              >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                    ...
                    .map((item, index) => (
                      ...
                      >
                        ...
                        <td className="py-2 text-red-300">
                          -${(item.amount * 6).toFixed(2)}
                        </td>
                        <td className="py-2 text-red-300">
                          -${(item.amount * 12).toFixed(2)}
                        </td>
                        <td className="py-2 text-red-300">
                          -${(item.amount * 36).toFixed(2)}
                        </td>
                        <td className="py-2 text-red-300">
                          -${(item.amount * 60).toFixed(2)}
                        </td>
                      </tr>
                    ))}
                  ...
                  {/* Net Total Row */}
                  {(() => {
                    const revenue6M = items
                      .filter((item) => item.type === "revenue")
                      .reduce((sum, item) => sum + item.amount * 6, 0);
                    const expenses6M = items
                      .filter((item) => item.type === "expense")
                      .reduce((sum, item) => sum + item.amount * 6, 0);
                    const net6M = (revenue6M - expenses6M).toFixed(2);

                    const revenue1Y = items
                      .filter((item) => item.type === "revenue")
                      .reduce((sum, item) => sum + item.amount * 12, 0);
                    const expenses1Y = items
                      .filter((item) => item.type === "expense")
                      .reduce((sum, item) => sum + item.amount * 12, 0);
                    const net1Y = (revenue1Y - expenses1Y).toFixed(2);

                    const revenue3Y = items
                      .filter((item) => item.type === "revenue")
                      .reduce((sum, item) => sum + item.amount * 36, 0);
                    const expenses3Y = items
                      .filter((item) => item.type === "expense")
                      .reduce((sum, item) => sum + item.amount * 36, 0);
                    const net3Y = (revenue3Y - expenses3Y).toFixed(2);

                    const revenue5Y = items
                      .filter((item) => item.type === "revenue")
                      .reduce((sum, item) => sum + item.amount * 60, 0);
                    const expenses5Y = items
                      .filter((item) => item.type === "expense")
                      .reduce((sum, item) => sum + item.amount * 60, 0);
                    const net5Y = (revenue5Y - expenses5Y).toFixed(2);

                    return (
                      <tr className="border-t-2 border-red-700 font-medium">
                        <td className="py-2 text-red-300">NET TOTAL</td>
                        <td
                          className={`py-2 ${(net6M as unknown as number) >= 0 ? "text-green-600/80" : "text-red-300"}`}
                        >
                          ${net6M}
                        </td>
                        <td
                          className={`py-2 ${(net1Y as unknown as number) >= 0 ? "text-green-600/80" : "text-red-300"}`}
                        >
                          ${net1Y}
                        </td>
                        <td
                          className={`py-2 ${(net3Y as unknown as number) >= 0 ? "text-green-600/80" : "text-red-300"}`}
                        >
                          ${net3Y}
                        </td>
                        <td
                          className={`py-2 ${(net5Y as unknown as number) >= 0 ? "text-green-600/80" : "text-red-300"}`}
                        >
                          ${net5Y}
                        </td>
                      </tr>
