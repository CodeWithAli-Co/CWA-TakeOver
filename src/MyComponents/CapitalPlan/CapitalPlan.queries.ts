...
async function fetchCapitalPlan(): Promise<CapitalPlanData> {
  const [
    ...
    companySupabase.from("capital_rounds").select("*").order("position", { ascending: true }),
    companySupabase.from("capital_checks").select("*").order("position", { ascending: true }),
    companySupabase.from("capital_check_touchpoints").select("*").order("occurred_at", { ascending: false }),
    companySupabase.from("capital_allocations").select("*").order("position", { ascending: true }),
    companySupabase.from("capital_line_items").select("*").order("position", { ascending: true }),
  ...
  useEffect(() => {
    const channel = companySupabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "capital_scenarios" },
        () => qc.invalidateQueries({ queryKey: CAPITAL_PLAN_QUERY_KEY }))
      .subscribe();
    return () => { companySupabase.removeChannel(channel); };
  }, [qc]);
}
...
export function useUpsertRound() {
  ...
  return useMutation({
    mutationFn: async (row: Partial<CapitalRound> & { name: string; round_type: CapitalRoundType }) => {
      const { data, error } = await companySupabase.from("capital_rounds").upsert(row).select().single();
...
export function useDeleteRound() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await companySupabase.from("capital_rounds").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: invalidate,
  });
...
export function useUpsertCheck() {
  ...
  return useMutation({
    mutationFn: async (row: Partial<CapitalCheck> & { round_id: string; investor_name: string }) => {
      const { data, error } = await companySupabase.from("capital_checks").upsert(row).select().single();
      if (error) throw error;
...
export function useDeleteCheck() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await companySupabase.from("capital_checks").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: invalidate,
  });
...
export function useMoveCheckStatus() {
  ...
  return useMutation({
    mutationFn: async ({ id, status }: { id: string; status: CapitalCheckStatus }) => {
      ...
      const { data, error } = await companySupabase.from("capital_checks").update(patch).eq("id", id).select().single();
      if (error) throw error;
      return data as CapitalCheck;
    },
    onSuccess: invalidate,
...
export function useAddTouchpoint() {
  ...
  return useMutation({
    mutationFn: async (row: Partial<CapitalCheckTouchpoint> & { check_id: string; summary: string }) => {
      const { data, error } = await companySupabase.from("capital_check_touchpoints").insert(row).select().single();
      if (error) throw error;
      // Also bump the check's last_touch_at
      await companySupabase.from("capital_checks").update({ last_touch_at: row.occurred_at ?? new Date().toISOString() }).eq("id", row.check_id);
...
export function useDeleteTouchpoint() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await companySupabase.from("capital_check_touchpoints").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: invalidate,
  });
...
export function useUpsertAllocation() {
  ...
  return useMutation({
    mutationFn: async (row: Partial<CapitalAllocation> & { round_id: string; bucket_name: string; category: CapitalCategory }) => {
      const { data, error } = await companySupabase.from("capital_allocations").upsert(row).select().single();
      if (error) throw error;
...
export function useDeleteAllocation() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await companySupabase.from("capital_allocations").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: invalidate,
  });
...
export function useUpsertLineItem() {
  ...
  return useMutation({
    mutationFn: async (row: Partial<CapitalLineItem> & { allocation_id: string; label: string }) => {
      const { data, error } = await companySupabase.from("capital_line_items").upsert(row).select().single();
      if (error) throw error;
...
export function useDeleteLineItem() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await companySupabase.from("capital_line_items").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: invalidate,
  });
...
export function useUpsertActual() {
  ...
  return useMutation({
    mutationFn: async (row: Partial<CapitalActual> & { amount: number }) => {
      const { data, error } = await companySupabase.from("capital_actuals").upsert(row).select().single();
      if (error) throw error;
...
export function useDeleteActual() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await companySupabase.from("capital_actuals").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: invalidate,
  });
...
export function useUpsertScenario() {
  ...
  return useMutation({
    mutationFn: async (row: Partial<CapitalScenario> & { name: string }) => {
      const { data, error } = await companySupabase.from("capital_scenarios").upsert(row).select().single();
      if (error) throw error;
...
export function useDeleteScenario() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await companySupabase.from("capital_scenarios").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: invalidate,
  });
