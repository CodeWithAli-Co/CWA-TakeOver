  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-y-auto">
      ...
      <div className="px-8 pt-7 pb-2">
        <div className="flex items-end justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[24px] font-bold text-foreground tracking-tight">Personal Life</h1>
      ...
      <Tabs
        ...
        className="px-8"
      >
        <TabsList className="mt-5 bg-muted/30 border border-border rounded-sm h-9 p-0.5">
          <TabsTrigger
            value="schedule"
            className="data-[state=active]:bg-primary/10 data-[state=active]:text-primary text-muted-foreground rounded-sm text-[12px] h-7 px-4 flex items-center gap-1.5"
          >
            ...
            <div className="flex">
              <div className="flex-1 px-5 py-4 border-r border-border">
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[10px] text-muted-foreground/60 uppercase tracking-[0.12em]">Today</span>
                </div>
                <p className="text-xl font-bold text-foreground tracking-tight">{todaysActivities.length}</p>
              </div>
              <div className="flex-1 px-5 py-4 border-r border-border">
                <div className="flex items-center gap-1.5 mb-1">
                  <Check className="h-3 w-3 text-emerald-500/60" />
                  <span className="text-[10px] text-muted-foreground/60 uppercase tracking-[0.12em]">Completed</span>
                </div>
              ...
              <div className="flex-1 px-5 py-4 border-r border-border">
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[10px] text-muted-foreground/60 uppercase tracking-[0.12em]">Upcoming</span>
                </div>
              ...
              <div className="flex-1 px-5 py-4">
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[10px] text-muted-foreground/60 uppercase tracking-[0.12em]">Reminders</span>
                </div>
                ...
                >
                  <div
                    className={`px-5 py-3 flex items-center justify-between border-b ${
                      isToday ? "border-red-500/10 bg-red-500/[0.03]" : "border-border"
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      {isToday && (
                      ...
                      <div>
                        <h2 className="text-[13px] font-semibold text-foreground/85 flex items-center gap-2">
                          {date.toLocaleDateString("en-US", { weekday: "long" })}
                          {isToday && (
                            <span className="text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded-sm bg-primary/10 text-primary border border-primary/15">
                              Today
                            </span>
                          )}
                        </h2>
                    ...
                    <div className="text-right">
                      <p className="text-[16px] font-bold text-foreground tracking-tight">{dayActivities.length}</p>
                  ...
                  <div className="divide-y divide-white/[0.025]">
                    {dayActivities.length === 0 ? (
                      <div className="py-6 text-center">
                        <p className="text-[12px] text-muted-foreground/60">No activities scheduled</p>
                      </div>
                    ) : (
        ...
        <TabsContent value="restaurants" className="space-y-4 mt-4 pb-6">
          ...
          <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center flex-wrap">
            <div className="flex-1 relative min-w-[200px]">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 bg-muted/30 border border-border rounded-sm text-[12px] text-foreground/60 placeholder:text-muted-foreground/40 focus:outline-none focus:border-border"
              />
            </div>

            <Select value={selectedCuisine} onValueChange={setSelectedCuisine}>
              <SelectTrigger className="w-full sm:w-[180px] bg-muted/30 border-border rounded-sm text-[12px] text-foreground/60 h-9">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-sm text-[11px] font-medium transition-colors ${
                showOnlyFavorites
                  ...
                  : "bg-muted/30 text-muted-foreground border border-border hover:text-foreground/60"
              }`}
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-primary/10 hover:bg-primary/15 border border-primary/20 text-primary text-[11px] font-medium rounded-sm transition-colors"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {filteredRestaurants.length === 0 ? (
              <div className="col-span-full bg-card border border-border rounded-sm py-16 text-center">
                <Utensils className="h-10 w-10 text-white/[0.05] mx-auto mb-3" />
                <p className="text-[14px] text-muted-foreground font-medium mb-1">No restaurants found</p>
...
const ActivityCard: React.FC<{
  ...
  return (
    <div className="px-5 py-3 hover:bg-card transition-colors group">
      <div className="flex items-start gap-3">
        {/* Time column */}
        <div className="text-[11px] font-mono text-muted-foreground min-w-[60px] pt-1 tabular-nums">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
            <h3
              className={`text-[13px] font-medium ${
                activity.completed ? "line-through text-muted-foreground" : "text-foreground/80"
              }`}
            >
              {activity.title}
            </h3>
          ...
          {activity.notes && (
            <p className="text-[10px] text-muted-foreground/50 italic">{activity.notes}</p>
          )}
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            ...
            className={`p-1.5 rounded-sm hover:bg-muted/50 transition-colors ${
              activity.completed ? "text-emerald-400" : "text-muted-foreground hover:text-emerald-400"
            }`}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="p-1.5 rounded-sm hover:bg-muted/50 text-muted-foreground hover:text-foreground/70 transition-colors">
                <Edit className="h-3.5 w-3.5" />
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="bg-card border-border rounded-sm">
              <DropdownMenuItem onClick={onEdit} className="text-[12px]">
                <Edit className="h-3 w-3 mr-2" /> Edit
              </DropdownMenuItem>
              <DropdownMenuItem onClick={onDelete} className="text-[12px] text-primary">
...
const RestaurantCard: React.FC<{
  ...
  return (
    <div className="bg-card border border-border hover:border-primary/10 rounded-sm p-4 transition-all group">
      {/* Header */}
      <div className="flex items-start justify-between mb-3 gap-2">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-xl">{cuisineInfo.icon}</span>
            <h3 className="text-[14px] font-semibold text-foreground/85 truncate">{restaurant.name}</h3>
          </div>
          <p className="text-[11px] text-muted-foreground">{cuisineInfo.label}</p>
        </div>

        <button
          onClick={() => onToggleFavorite(restaurant)}
          className={`p-1.5 rounded-sm hover:bg-muted/50 transition-colors shrink-0 ${
            restaurant.is_favorite ? "text-primary" : "text-muted-foreground hover:text-primary"
          }`}
      ...
      <div className="space-y-1.5 mb-3">
        <div className="flex items-start gap-1.5 text-[12px] text-muted-foreground/80">
        ...
        {restaurant.website && (
          <div className="flex items-center gap-1.5 text-[12px]">
            ...
            <a
              ...
              className="text-primary/70 hover:text-primary truncate"
            >
              Website
            </a>
          </div>
      ...
      {/* Notes */}
      {restaurant.notes && (
        <div className="bg-card border-l-2 border-amber-500/30 px-2 py-1 mb-3 rounded-sm">
          <p className="text-[11px] text-muted-foreground/80 italic leading-snug">{restaurant.notes}</p>
        </div>
      )}
      ...
      {restaurant.tags && restaurant.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          {restaurant.tags.map((tag) => (
            <span
              key={tag}
              className="text-[10px] px-1.5 py-0.5 bg-muted/40 rounded-sm text-muted-foreground/70"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="flex items-center gap-1.5 pt-3 border-t border-border opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          onClick={onEdit}
          className="flex-1 flex items-center justify-center gap-1 py-1.5 bg-muted/30 hover:bg-muted/50 border border-border text-muted-foreground/70 hover:text-foreground/80 text-[11px] rounded-sm transition-colors"
        >
          <Edit className="h-3 w-3" /> Edit
        </button>
        <button
          onClick={onDelete}
          className="px-2.5 py-1.5 bg-muted/30 hover:bg-primary/[0.06] border border-border hover:border-primary/15 text-muted-foreground/70 hover:text-primary rounded-sm transition-colors"
          title="Delete"
        >
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <Input
              ...
              className="bg-muted/30 border-border text-foreground/80 rounded-sm focus:border-primary/20"
            />
          </div>
          ...
          <div>
            ...
            >
              <SelectTrigger className="bg-muted/30 border-border text-foreground/80 rounded-sm focus:border-primary/20">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                ...
                {DAYS.map((day, index) => {
                  ...
                  return (
                    <Button
                      ...
                      className={
                        isSelected
                          ? "bg-primary hover:bg-primary/80 rounded-sm"
                          : "border-border"
                      }
                    >
                      {day}
        ...
        <DialogFooter>
          ...
          <Button
            ...
            className="bg-primary hover:bg-primary/80 rounded-sm"
          >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {formData.tags?.map((tag) => (
                <span
                  key={tag}
                  className="text-xs px-2 py-1 bg-muted/50 rounded-sm text-muted-foreground/80 flex items-center gap-1"
                >
                  {tag}
                  <X
