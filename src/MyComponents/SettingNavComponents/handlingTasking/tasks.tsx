...
import { ActiveUser, Todos, TodosInterface } from '@/stores/query';

// Component for displaying task blockers
const TaskBlockerItem: React.FC<{ blocker: TaskBlocker }> = ({ blocker }) => (
  <div className={`p-3 rounded-lg border ${
    blocker.status === 'active' ? 'border-red-500/30 bg-red-950/10' : 'border-green-500/30 bg-green-950/10'
  }`}>
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2">
        ...
        <span className={`text-sm ${blocker.status === 'active' ? 'text-red-200' : 'text-green-200'}`}>
          {blocker.description}
        </span>
      </div>
      <Badge variant="outline" className={`
        ${blocker.severity === 'critical' ? 'bg-red-500/20 text-red-400' :
          blocker.severity === 'moderate' ? 'bg-yellow-500/20 text-yellow-400' :
...
const TaskDependencyItem: React.FC<{ dependency: TaskDependency }> = ({ dependency }) => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <h4 className="text-sm font-medium text-red-200">Description</h4>
...
const TaskSettings: React.FC = () => {
  ...
  const getTaskStats = () => {
    ...
    const inProgress = localTasks.filter(t => t.status === 'in-progress').length;
    const todo = localTasks.filter(t => t.status === 'to-do').length;
  ...
  return (
    ...
    {todos.map((Todo) => (
          ...
          <div className="mb-6">
            ...
            <p className="text-red-200/60">{Todo.allCount} total tasks</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-6">
            <Card className="bg-black/40 border-red-950/20">
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold text-red-200 mt-2">{Todo.doneCount}</p>
              </CardContent>
            </Card>
            <Card className="bg-black/40 border-red-950/20">
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold text-red-200 mt-2">{Todo.inProgressCount}</p>
              </CardContent>
              </Card>
            <Card className="bg-black/40 border-red-950/20">
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold text-red-200 mt-2">{Todo.todoCount}</p>
              </CardContent>
            </Card>
            <Card className="bg-black/40 border-red-950/20">
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold text-red-200 mt-2">{Todo.allCount}</p>
              </CardContent>
            </Card>
          </div>
              ...
              <Tabs defaultValue="to-do" className="mb-6">
                <TabsList className="bg-black/40 border border-red-950/20">
                  ...
                  <TabsTrigger 
                    ...
                    className="data-[state=active]:bg-red-900/20 data-[state=active]:text-red-200"
                  >
                    In Progress ({Todo.inProgressCount})
                  </TabsTrigger>
                  <TabsTrigger 
                    value="done"
