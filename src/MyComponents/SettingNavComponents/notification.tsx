  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-amber-50">Notification Settings</h2>
          <p className="text-sm text-amber-50/70">Manage your notification preferences and alerts</p>
        </div>
        <Button 
          variant="outline" 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-black/40 border-red-900/30">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-amber-50">Email Notifications</CardTitle>
          ...
          <CardContent>
            <div className="space-y-4">
              {[
                { title: "Broadcast Updates", description: "Get notified about broadcast status" },
                { title: "Contact Changes", description: "Notifications for contact list updates" },
                { title: "System Alerts", description: "Important system notifications" },
                { title: "Weekly Reports", description: "Weekly summary and analytics" }
              ].map((item, i) => (
                <motion.div
                  key={i}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-amber-50/70">{item.description}</p>
                    </div>
                  </div>
                  <Switch className="data-[state=checked]:bg-red-900" defaultChecked />
                </motion.div>
              ))}
            </div>
          </CardContent>
        ...
        <Card className="bg-black/40 border-red-900/30">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-amber-50">In-App Notifications</CardTitle>
              <Badge variant="outline" className="bg-emerald-500/20 text-emerald-400">
                Enabled
              </Badge>
            </div>
          ...
          <CardContent>
            <div className="space-y-4">
              {[
                { title: "Push Notifications", description: "Enable desktop notifications", icon: Bell },
                { title: "Chat Alerts", description: "New message notifications", icon: MessageSquare },
                { title: "Critical Alerts", description: "High-priority notifications", icon: AlertCircle },
                { title: "Security Alerts", description: "Security-related notifications", icon: Shield }
              ].map((item, i) => (
                <motion.div
                  key={i}
                  initial={{ opacity: 0, y: 10 }}
       ...
       <Card className="bg-black/40 border-red-900/30 col-span-2 ">
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-amber-50">Recent Notifications</CardTitle>
                <Button 
                  variant="ghost" 
                  size="sm"
