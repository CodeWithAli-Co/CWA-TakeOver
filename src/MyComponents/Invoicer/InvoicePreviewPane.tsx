...
export const InvoicePreviewPane: React.FC<Props> = ({ invoiceId, onClose }) => {
  ...
  return (
    <div className="bg-card border border-border rounded-sm h-full flex flex-col overflow-hidden">
      {/* Header */}
      <div className="px-4 pt-3 pb-3 flex items-center justify-between border-b border-border">
        <div className="flex items-center gap-2 min-w-0">
          <div className="p-1.5 rounded-sm bg-primary/[0.08]">
            <FileText className="h-3.5 w-3.5 text-primary" />
          </div>
          <div className="min-w-0">
            <p className="text-[11px] text-muted-foreground/40 uppercase tracking-[0.15em] font-medium">
              Preview
            </p>
            {invoice && (
        ...
        <div className="flex items-center gap-1.5">
          {invoice && (
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 bg-primary/[0.08] hover:bg-primary/80/[0.12] border border-primary/15 text-primary text-[11px] rounded-sm transition-colors"
              title="Download PDF"
            >
