...
const StatCard = ({
  ...
  <motion.div
    ...
    className="bg-zinc-950 high-dpi:bg-zinc-950/20 border border-red-900/30 rounded-lg p-4 hover:border-red-800/50 transition-colors"
  >
  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        ...
        if (meetingError) {
          console.log(
            "There was an error with fetching the meetings Id from supabase",
            meetingError.message
          );
        }
        if (credentialsError) {
          console.log(
            "There was an error retrieving credential data from supabase",
            credentialsError.message
          );
        }
        if (taskError) {
          console.log(
            "There was an error with fetching tasks",
            taskError.message
          );
        }
        if (revenueError)
        ...
        if (expenseError)
          console.log(
            "There was an error retrieving the expenses from the database",
        ...
        if (allTask) {
          ...
          const userRoleTasks = allTask.filter(
            (task) => task.label === userRole
          ).length;

          // count the completedd aand pending tasks for the user roles
          // might changee allTask.filter to userRoleTask.filter
          const completed = allTask.filter(
            (task) => task.status === "done"
          ).length;
          const pending = allTask.filter(
        ...
        if (credentials) {
          ...
          if (userRole === Role.CEO || userRole === Role.COO) {
            ...
            userRole === Role.AccManager
          ) {
            accountsToShow = credentials.filter(
              (cred) => cred.folder === "default"
            ).length;
          } else {
            accountsToShow = 0;
          }
  ...
  return (
    <>
      ...
      >
        ...
        {/* ACCOUNTS  */}
        <UserView
          userRole={[Role.CEO, Role.COO, Role.AccManager, Role.Marketing]}
        >
          <StatCard label="Accounts" value={accounts} icon={FolderCode} />
        </UserView>

        {/* MEETINGS */}
        <StatCard label="Meetings" icon={Video} value={meeting} />
      </motion.div>
    </>
  );
};
