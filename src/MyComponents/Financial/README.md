...
Currently the modeler reads from but does not write to companySupabase.To add write-back:

1. Open `FinancialContext.tsx`
2. In each setter (e.g. `setInitialCapital`), add an `await companySupabase.from('cwa_calculatorProps').upsert({ ... })` call
