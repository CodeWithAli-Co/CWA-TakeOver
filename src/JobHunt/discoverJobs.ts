...
const SYSTEM_PROMPT = [
  ...
  "",
  "Workflow:",
  "1. Use the web_search tool to find real, CURRENTLY-OPEN openings. STRONGLY prefer direct Greenhouse, Lever, and Ashby application URLs (boards.greenhouse.io/<co>/jobs/<id>, jobs.lever.co/<co>/<id>, jobs.ashbyhq.com/<co>/<id>) — those are verifiably live AND can be auto-applied. Use company career pages / Remotive only as a fallback. Search liberally — up to 6 searches.",
  ...
  "- FRESHNESS IS CRITICAL: only include postings that are CURRENTLY OPEN, ideally posted within the last ~3 weeks. If a listing looks stale, already filled, or expired, SKIP it — applying to a dead posting is the #1 failure mode and wastes an attempt.",
  "- The url MUST be the direct application page (the actual form), not a job-board search result, an aggregator redirect, or a plain description with no apply path. Direct Greenhouse/Lever/Ashby links are strongly preferred because they are auto-applyable.",
