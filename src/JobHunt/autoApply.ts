...
export interface ApplyResult { status: "submitted" | "needs_human" | "manual" | "error"; reason: string; applyUrl?: string | null; ats: AtsKind; screenshotBase64?: string }
...
export async function autoApply(job: ApplyJobInput, profile: ApplyProfile, masterResume: string): Promise<ApplyResult> {
  ...
  if (!ref.canAutoApply) {
    return { status: "manual", reason: `${ATS_LABEL[ref.kind]} needs the human apply form.`, applyUrl: job.url, ats: ref.kind };
  }
  ...
  try {
    ...
    const res = await fetch(endpoint(), {
      ...
      body: JSON.stringify({
        ...
        profile, resumeBase64, resumeFilename: safeFile(`Resume - ${job.company}`) + ".pdf", answers: {},
      }),
    });
    if (!res.ok) return { status: "error", reason: `Apply API ${res.status}: ${(await res.text().catch(() => "")).slice(0, 140)}`, ats: ref.kind };
    const d = (await res.json()) as { status?: string; reason?: string; applyUrl?: string; screenshotBase64?: string };
    return { status: (d.status as any) || "manual", reason: d.reason || "", applyUrl: d.applyUrl ?? job.url, ats: ref.kind, screenshotBase64: d.screenshotBase64 };
  } catch (e: any) {
